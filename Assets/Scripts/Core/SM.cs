...
namespace TC
{
    ...
    public class SM : MonoBehaviour
    {
        ...
        private static void LoadGameManager()
        {
            ...
            for (var i = 1; i < GM.db.data[0].sceneName.Length; i++)
            {
                if (GM.db.data[0].sceneName[i] == "")
                    continue;

                SceneManager.LoadScene(/*"Scenes/" + */GM.db.data[0].sceneName[i], LoadSceneMode.Additive);
            }
        }
        ...
        {
            // ロードScene
            for (var i = 0; i < GM.db.data[(int)loadScene].sceneName.Length; i++)
            {
                var val = GM.db.data[(int)loadScene].sceneName[i];

                if (val == null || (i < GM.db.data[previous].sceneName.Length &&
                    val == GM.db.data[previous].sceneName[i]))
                    continue;
            ...
            if (previous != 0)
            {
                for (var i = 0; i < GM.db.data[(int)previous].sceneName.Length; i++)
                {
                    var val = GM.db.data[(int)previous].sceneName[i];

                    if (val == null || (i < GM.db.data[(int)loadScene].sceneName.Length &&
                        val == GM.db.data[(int)loadScene].sceneName[i]))
                        continue;
